...
https://github.com/gnif/LookingGlass/blob/master/host/TextureConverter.cpp
*/

#include "TextureConverter.h"
#include "src/log/log.h"

#include "src/texture_converter/shaders/Pixel.h"
#include "src/texture_converter/shaders/Vertex.h"
#include "src/texture_converter/shaders/RGBtoYUV.h"
...
)
{
  ...
  result = device->CreatePixelShader(Pixel_PX_main, sizeof(Pixel_PX_main), NULL, &m_psCopy);
  if (FAILED(result))
  {
    DeInitialize();
    log_error("Failed to create the copy pixel shader");
    return false;
  }
  ...
  switch (format)
  {
    case FRAME_TYPE_YUV420:
      result = device->CreatePixelShader(RGBtoYUV_PX_main, sizeof(RGBtoYUV_PX_main), NULL, &m_psConversion);
  ...
  {
    DeInitialize();
	log_error("Failed to create the pixel shader");
    return false;
  }
  ...
  result = device->CreateInputLayout(inputDesc, _countof(inputDesc), Vertex_VX_main, sizeof(Vertex_VX_main), &m_layout);
  if (FAILED(result))
  {
    DeInitialize();
	log_error("Failed to create the input layout");
    return false;
  }

  result = device->CreateVertexShader(Vertex_VX_main, sizeof(Vertex_VX_main), NULL, &m_vertexShader);
  if (FAILED(result))
  {
    DeInitialize();
	log_error("Failed to create the vertex shader");
    return false;
  }
  ...
  for(int i = 0; i < _countof(m_targetTexture); ++i)
  {
    ...
    {
      DeInitialize();
	  log_error("Failed to create the render texture");
      return false;
    }
    ...
    {
      DeInitialize();
	  log_error("Failed to create the render view");
      return false;
    }
    ...
    {
      DeInitialize();
	  log_error("Failed to create the resource view");
      return false;
    }
  }
...
bool TextureConverter::Convert(ComPtr<ID3D11Texture2D> texture, TextureList & output)
{
  ...
  for (int i = 0; i < _countof(m_renderView); ++i)
  {
    ...
    result = m_device->CreateRenderTargetView(dest.Get(), &targetDesc, &view);
    if (FAILED(result))
    {
      delete[] renderViews;
      DeInitialize();
