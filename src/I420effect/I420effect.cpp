...
uint32_t I420Effect::GetDisplayedFrameWidth() const { return m_displayedFrame.width; }
HRESULT I420Effect::SetDisplayedFrameWidth(uint32_t width)
{
	m_displayedFrame.width = width;
	return S_OK;
}

uint32_t I420Effect::GetDisplayedFrameHeight() const { return m_displayedFrame.height; }
HRESULT I420Effect::SetDisplayedFrameHeight(uint32_t height)
{
	m_displayedFrame.height = height;
	return S_OK;
}

uint32_t I420Effect::GetVisibleWidth() const { return m_visibleSize.width; }
HRESULT I420Effect::SetVisibleWidth(uint32_t width)
{
	m_visibleSize.width = width;
	return S_OK;
}

uint32_t I420Effect::GetVisibleHeight() const { return m_visibleSize.height; }
HRESULT I420Effect::SetVisibleHeight(uint32_t height)
{
...
HRESULT I420Effect::Register(_In_ ID2D1Factory1* pFactory)
{
	...
	// Convert type of XML value from <char*> to <PCWSTR>
	const size_t WCHARBUF = 3000;
	wchar_t  pszXml_PCWSTR[WCHARBUF];
	MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pszXml, -1, pszXml_PCWSTR, WCHARBUF);
...
{
	HRESULT hr = pEffectContext->LoadPixelShader(GUID_I420PixelShader, PIXEL_SHADER_main, ARRAYSIZE(PIXEL_SHADER_main));
	if (SUCCEEDED(hr))
	{
		hr = pEffectContext->LoadVertexShader(GUID_I420VertexShader, VERTEX_SHADER_main, ARRAYSIZE(VERTEX_SHADER_main));
		...
		if (SUCCEEDED(hr))
		{
			...
			if (m_vertexBuffer == nullptr)
			{
				...
				cvbProp.shaderBufferWithInputSignature = VERTEX_SHADER_main;
				cvbProp.shaderBufferSize = ARRAYSIZE(VERTEX_SHADER_main);
...
IFACEMETHODIMP I420Effect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		m_drawInfo->SetInstructionCountHint(sizeof(PIXEL_SHADER_main));
	}
